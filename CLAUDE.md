...
- Category icon slugs (`utensils`, `coffee`, `wine`, `shopping-bag`, `car`, `home`, `heart-pulse`, `heart-handshake`, `film`, `plane`, `gift`, `more-horizontal`) map to lucide icons in `lib/utils/category-icon.tsx`. The legacy `bus` slug is aliased to the `Car` icon so historical rows still render. When you add a seed category, add its slug there too.

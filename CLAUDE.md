...
- **Budget cycle**: the user is only ever asked **when money comes in** (label reads `돈 들어오는 날` — deliberately persona-neutral covering 월급/용돈/입금, *not* the salaried-only `월급날`; settings has a single Select: `1일`~`28일` + `말일`), and the app maps it to `(cycle_mode, cycle_start_day)` on `user_settings` — `1일`/`말일` → `calendar` (`startDay=1`; a month-end paycheck funds the *next* calendar month so the tracking window IS the calendar month, which sidesteps the old short-month 31-clamp drop bug), `2~28` → `income_day` (`startDay=N`). Mapping helpers are `paydayToCycle`/`cycleToPayday`/`PAYDAY_OPTIONS` (single source) in `lib/utils/calendar.ts`; the form reduces to two hidden `cycle_mode`/`cycle_start_day` inputs in `components/settings/settings-form.tsx`. The DB columns + domain (`calendar|income_day`, `startDay` 1–31, default `calendar`/1), the `get_user_cycle` RPC, the server-side validation (still accepts 1–31), and the migrations are **unchanged** (backward compatible) — legacy `income_day` 29~31 rows surface as `말일` in the form and keep their DB value until the user saves, at which point they are upgraded to `calendar`/1. The dashboard reads both modes, the calendar grid is fixed 7×6 in calendar mode and a variable-row grid in income_day mode, and the MonthSwitcher label switches between `5월` and `5/20 – 6/19`. `getCycleRange`/`clampDayToMonth` stay untouched (the clamp path is retained only for legacy income_day rows; new UI never generates `startDay` > 28 so it is unreachable on the happy path).

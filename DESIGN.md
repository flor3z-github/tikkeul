...
```txt
/dashboard  현재 소비 상태 확인
/settings   월 수입과 고정지출 설정
...
하단 탭 네비게이션은 MVP에서 사용하지 않는다.

대신 소비 추가 Floating Action Button을 화면 오른쪽 아래에 고정한다.
...
MVP에서는 bottom navigation이 없으므로 glass bottom nav 스타일은 사용하지 않는다.
...
### 11.2 No Bottom Navigation in MVP

MVP에서는 하단 탭 네비게이션을 사용하지 않는다.

이유:

- 화면 목적이 단순하다.
- 주요 행동은 소비 추가 하나다.
- 하단 탭은 정보 구조를 불필요하게 키운다.

대신 홈 화면에 다음 진입점만 제공한다.

...
- 카테고리 Top 5
- 복잡한 차트 / 미니 그래프 (추세도 막대·그래프로 그리지 않는다 — 텍스트 한 줄만, §12.2)
- 오늘 지출 카드
- 수입 내역
- 분석 그래프
- 지난달 대비 비교 문구 — **예외: 총액 추세 페이스 한 줄**(§12.2). "지난 주기 같은 때보다 N만원 더/덜 쓰는 중"만 허용하고, 그 이상의 비교 카드·항목별 전월比·막대/그래프는 두지 않는다(항목별 전월比는 /stats §12.9의 일).
...
총액 추세 라인 (토스式 페이스 한 줄):

- hero 숫자("쓴 돈") 아래, progress bar 위에 **텍스트 한 줄**로 둔다 — "지난 {주기|달} 같은 때보다 N만원 더/덜 쓰는 중". `차이 < 5,000원`이면 "비슷하게 쓰는 중". 막대·그래프·미니그래프는 두지 않는다(§12.1/§19) — hero 숫자와 progress 막대만으로도 카드가 충분히 빽빽하므로 추세는 글로만 거든다.

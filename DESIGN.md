...
합계 카드는 탭 가능한 진입점이다 — own 모드에서 카드를 탭하면 `/stats`(§12.9, 이번 사이클 소비 구성 분해)로 이동한다. 친구 모드에서는 탭 동작과 affordance를 모두 숨긴다. 카드 내부 숫자·progress 시각 계층은 그대로 두고 탭 영역만 카드 전체에 얹는다.
...
- 진입점은 대시보드 합계 카드 탭 하나다 (§12.2, §11.2). 하단 탭에 추가하지 않는다 (2탭 유지, §19).
- own 모드 전용. 친구 모드(`?viewing=`)에서는 진입점을 숨긴다 — 통계는 `monthly_income` 파생값을 쓰지 않지만, 친구 소비 구성을 분해하는 surface는 §12.8 노출 범위를 벗어나므로 두지 않는다.
- 사이클·라벨은 대시보드와 동일 payday 엔진(§12.5a) 재사용. v1은 현재 사이클만 본다(스위처 없음).

권장 구조:

```txt
Header  ← "통계" / eyebrow 없음
총 소비  920,000원   (변동 742,000 · 고정 178,000)
...
- **변동 섹션**: `transactions`를 `category_id`로 집계. 행 = 아이콘·카테고리명·금액·CSS 막대·%. 막대·%는 변동 섹션 내 비중(자기 정규화, 합 100%). 금액 desc, 0 카테고리 숨김, `deleted_at` 제외.
- **고정 섹션**: `get_fixed_effective_items(target, ym)`의 항목별 effective 금액. 행 = 아이콘(`fixed-category-icon`)·이름/`plan_name`·금액. `is_overridden`이면 base 대비 차이를 "↑/↓ 이번달 ±N"(`amount − base_amount`)으로 표기. 막대 없음. 금액 desc.
- 비대칭은 의도된 것 — 변동은 거래가 많아 카테고리로 묶고, 고정은 항목이 적고 각각이 정체성이라 항목별로 펼친다(은행 이자처럼 override된 변동을 항목 단위로 보기 위함).
- override는 변동을 자동 감지하지 않는다 — 사용자가 그 사이클에 override를 입력했을 때만 effective/표기에 반영되고, 없으면 base 그대로(변동 표기 없음)가 정상이다.

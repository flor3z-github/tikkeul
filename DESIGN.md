...
홈 화면에서 가장 먼저 보여야 하는 것은 이번 달 "쓴 돈"이다. 좌·우 grid의 왼쪽 자리에 가장 큰 hero 숫자로 배치한다. 오른쪽 자리에는 "남은 돈"이 한 단계 작고 muted 톤으로 따라붙어, 한눈에 비교는 가능하되 시선은 "쓴 돈"으로 먼저 향하게 한다. 카드 상단 eyebrow ("이번 달" 등)는 두지 않는다 — MonthSwitcher 라벨(`5월` / `5/20 – 6/19`)이 사이클 맥락을 이미 전담한다.
...
쓴 돈                    남은 돈
314,200원               1,885,800원
[32px extrabold]        [22px medium muted]
...
- 합계 카드(`MonthlySummaryCard`)를 탭하면 `/stats`로 이동한다 (§12.2, §12.9). own 모드에서만 동작하며 친구 모드에서는 진입점을 숨긴다.
...
가장 중요한 카드다. 좌측에 "쓴 돈"이 hero 숫자(32px extrabold), 우측에 "남은 돈"이 그보다 한 단계 작고 muted한 숫자(22px medium muted)로 배치된다. "쓴 돈"이 1차 시선 대상이고 "남은 돈"은 같은 라인에서 즉시 비교 가능한 보조 숫자다. 사용률 %는 월 수입 대비로 계산하며 progress bar 옆에 보조 라벨로 둔다. 카드 상단 eyebrow는 두지 않으며 (MonthSwitcher가 사이클 맥락 전담), 상태 카피는 정상 구간에서는 표시하지 않는다. 카드 하단에는 점선 구분선으로 페이스 라인을 분리해 "남은 N일 · 하루 X원"을 노출한다 — 사용자가 "남은 돈"을 즉시 일일 행동 단위로 환산하기 위함이다.
...
● 고정 13,000원   ● 소비 301,200원
- - - - - - - - - - - - - - - - - - - - - - - -
이번 달이 끝나기까지        남은 12일 · 하루 140,483원
```
...
- 친구 모드에서는 페이스 라인을 절대 노출하지 않는다 (남은 돈은 `monthly_income`에서 파생되므로 §19와 충돌).
...
`남은 돈`이 음수이면 우측 열의 라벨은 "초과"로 바뀌고 destructive 색을 쓰며, 표시 값은 절댓값이다.
...
정상 구간에서는 progress bar 색과 `N% 사용` 라벨 색이 시그널 전부를 담당한다. 주의/위험/초과 구간에서만 카드 하단에 "주의 구간" / "위험 구간" / "예산 초과" 카피를 한 줄 노출한다.
...
합계 카드는 탭 가능한 진입점이다 — own 모드에서 카드를 탭하면 `/stats`(§12.9, 이번 사이클 소비 구성 분해)로 이동한다. 친구 모드에서는 탭 동작과 affordance를 모두 숨긴다. 카드 내부 숫자·progress 시각 계층은 그대로 두고 탭 영역을 카드 전체에 얹되, **발견성을 위해 카드 하단에 구분선 + 「📊 소비 구성 보기 ›」 subtle 행으로 진입 단서**를 둔다(현재 사이클에서 진입 가능할 때만; 과거 사이클·친구 모드에서는 단서도 숨긴다). 탭은 stretched-link 오버레이가 받으므로 이 단서는 시각 표시일 뿐이다.
...
친구 모드 거래 row에는 인라인 반응 카운트나 댓글 프리뷰가 없다 — 친구가 보낸 인터랙션은 DM에 모이므로 viewer는 자기가 남긴 흔적(`lastEmoji`/`lastComment` trace)만 보고, 그 외엔 DM 스레드에서 확인한다 (정책 결정 2).

**자기 거래에 달린 댓글 (owner 대시보드)**

정책 결정 2의 예외로, **owner 자기 대시보드**에서는 친구가 내 거래에 남긴 **댓글(텍스트)** 을 거래 row 아래 trace 한 줄로 노출한다 — `{닉네임} · {댓글}` + `MessageCircle` 아이콘, `/dm/<보낸친구>?message=<id>` 딥링크. unread(해당 DM 스레드를 마지막으로 읽은 이후 도착)면 primary로 강조 + 점, 읽은 댓글은 muted로 계속 표시한다. DM 스레드 방문 시 `mark_dm_thread_read`가 호출되어 다음 대시보드 로드에서 강조가 자동 해제된다.
...
- 진입점은 대시보드 합계 카드 탭 하나다 (§12.2, §11.2). 카드 하단의 「📊 소비 구성 보기 ›」 subtle 행이 진입 단서이며, 탭은 카드 전체가 받는다. 하단 탭에 추가하지 않는다 (2탭 유지, §19).
...
- 헤더 eyebrow에 「← 대시보드」 back-link을 둔다(`/settings` 선례와 동일). own 전용이라 하단 2탭이 없고 iOS PWA standalone엔 브라우저 back이 없어 나갈 경로가 필요하다 — 이 한 화면에 한해 §3.3의 "eyebrow 없음" 원칙보다 back-nav를 우선한다. 본문 상단 총액은 「{사이클} 총 소비」로 어느 사이클인지 함께 보인다.
...
- **전월比 규칙**: 상단 총액·변동 카테고리별·고정 항목별 모두 `이번 − 직전`을 "↑/↓ ±N"으로 표기한다(더 씀=↑ warning, 덜 씀=↓ muted). 단 **직전 사이클에 실제 거래가 있을 때만**(`hasPrevBaseline`) 전체 전월比를 켠다 — 고정지출은 standing 레코드라 직전 effective가 항상 존재하므로, 첫 사이클 유저에게 가짜 전월比가 뜨지 않도록 직전 거래 존재를 게이트로 둔다. 직전에 없던 신규 항목/카테고리, 안 변한 항목(delta 0)은 표기하지 않는다. 고정 전월比·상단 전월比는 두 고정 RPC가 모두 성공했을 때만 켠다(실패 시 prev가 0으로 읽혀 delta가 부풀려지는 것 차단).
- **같은 경과 시점 비교(변동 한정)**: 이번 사이클은 진행 중이라 "지금까지" 쓴 부분합인데 직전은 완료된 전체합이다 — 그대로 빼면 사이클 초반엔 항상 "덜 썼다(↓)"고 나오는 사과-오렌지 비교가 된다. 그래서 **직전 사이클 변동지출(상단 총액의 변동분 + 카테고리별 모두)을 이번 사이클이 지난 경과 시간만큼만 잘라**(`cutoff = prevStart + (now − cycleStart)`) 같은 시점끼리 비교한다(`clampToElapsedWindow`). **고정지출은 자르지 않는다** — 결제일 하루에 몰린 step이라 같은 컷오프를 적용하면 직전 결제가 컷 밖으로 잘려 가짜 +증가가 뜨는 timing artifact가 생긴다(고정은 양쪽 전액 비교). 이번 사이클 측은 자르지 않는다(상단 총액이 대시보드 `쓴 돈`과 정확히 같아야 하는 불변식 보존). 비교가 "전체 대비"가 아니라 "같은 시점까지"임을 드러내려고, 상단 전월比 라벨("지난 사이클보다 ↑ N원") **옆에 작게 「M/D 기준」**(M/D = 오늘=계산 기준 시점)을 muted로 붙인다.
...
- 통계 화면을 하단 탭으로 만들지 않는다 — `/stats` 진입은 대시보드 합계 카드 탭 하나이며 탭은 2개를 유지한다 (§11.2, §12.9).

...
`남은 돈`이 음수이면 2분할 좌측 "남은 돈" 셀의 라벨이 "초과"로 바뀌고 destructive 색을 쓰며, 표시 값은 절댓값이다.
...
합계 카드는 탭 가능한 진입점이다 — own 모드에서 카드를 탭하면 `/stats`(§12.9, 이번 사이클 소비 구성 분해)로 이동한다. 친구 모드에서는 탭 동작과 affordance를 모두 숨긴다. 카드 내부 숫자·progress 시각 계층은 그대로 두고 탭 영역을 카드 전체에 얹되, **발견성을 위해 카드 하단에 채운 회색 알약(`bg-muted`) 버튼 「◐ 소비 구성 보기 ›」로 진입 단서**를 둔다(현재 사이클에서 진입 가능할 때만; 과거 사이클·친구 모드에서는 단서도 숨긴다). 탭은 카드 전체를 덮는 stretched-link 오버레이가 받으므로 이 알약은 시각 표시일 뿐이다.
...
1. **합계 카드** — "이번 달 고정지출" 라벨 + 우상단 개수 칩(파란 점 + "총 N개 항목") + 큰 금액(40px) + **연간 환산** 행(`합계 × 12`, recessed plane `bg-muted/60` 위 보조 숫자). 연간 환산은 핵심 숫자(월 합계)보다 작은 weight·muted로 눌러 월 숫자가 단일 지배 figure로 남게 한다(§3). 금액 미입력 항목은 0으로 합산돼 연환산도 불완전할 수 있다(월 합계와 동일 한계).
...
- **HERO 카드**: 라벨 「이번 달 모은 돈」 + 큰 금액(검정, §3 핵심 숫자) + 「원」. 보조 문구 "쓴 게 아니라 **다시 내 자산이 되는 돈**이에요"(강조부 primary). 그 아래 풀폭 박스 「올해 모은 돈」(연초 이후 누적, primary tint 배경).
- **섹션 A — 투자·자유 적립**: 목표·만기가 **없는** 항목. 진행률 막대 없이 월 적립액 + 연 환산만 표시한다(만기가 없어 목표 대비 진행률이 성립하지 않음). 섹션 헤더 우측 라벨 「만기 없이 모으는 중」.
- **섹션 B — 달성형 목표**: 목표 금액 **또는** 만기일이 있는 항목. 진행률 막대(트랙 `bg-muted` / 채움 primary) + 진행률 % + 남은기간 라벨 + 「{현재} / {목표}원」. 섹션 헤더 우측 라벨 「목표까지 진행률」.
- **FAB**: 오른쪽 아래 추가 버튼(대시보드 FAB와 동일 위치·primary). 적금/투자/목표 추가 BottomSheet 진입.

원칙:

- 항목의 섹션은 **파생값**으로 가른다 — `goal_amount` 또는 `maturity_date`가 있으면 섹션 B(달성형 목표), 없으면 섹션 A(자유 적립). 별도 type 플래그를 두지 않는다.
- "모은 돈"은 저장값이 아니라 **파생값**이다 — `start_date`(첫 적립월)부터 `월 적립액 × 경과 적립 횟수`로 계산한다(`current_amount` 컬럼 없음). 중도 금액 변경·해지 정밀 반영은 범위 밖(선형 누적, `lib/utils/savings.ts`).
- 남은기간: 만기 있으면 「만기까지 N개월」, 목표금액형이면 「목표까지 약 N개월」 = ⌈(목표−현재)/월적립액⌉.
...
향후(Phase 2): 대시보드 히어로를 「나간 돈 = 모으기 + 고정비 + 소비」 3분할로 바꾸고(저축은 "쓴 돈"·소비율에서 제외, `remainingBudget`에서는 차감), 친구 노출은 공개/비공개 권한 토글로 확장한다.

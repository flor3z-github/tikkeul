...
하단 탭 네비게이션은 다음 4탭만 사용한다 (§11.2 참조).
...
```txt
[ 소비 ]      [ 고정지출 ]      [ 돈모으기 ]   [ 수입 ]
 /dashboard    /fixed-expenses   /savings       /income
```

원칙:

- 탭은 항상 4개다 (소비 / 고정지출 / 돈모으기 / 수입). 설정·통계·친구는 탭에 넣지 않는다 — 5번째 탭을 추가하지 않는다.
- 설정은 대시보드 우상단 아이콘으로 진입한다.
- 활성 탭은 아이콘에 primary 색을 적용하고 라벨은 foreground로, 비활성 탭은 muted-foreground로 표시한다. 추가로 활성 탭에는 `bg-primary/10` 배경 하이라이트가 opacity 크로스페이드로 나타난다 — 확장 상태에선 레일 안에 사방 8px 균일 inset으로 떠 있는 스타디움 pill(폭 = 슬롯 − 16px, 높이 48px, `rounded-full`; radius 24 = 레일 32 − inset이라 코너 갭이 동심으로 균일), 축소 상태에선 아이콘 중심에 맞춘 36px 원. 폭은 라벨 길이와 무관하며, 슬라이딩 없이 탭별 독립 페이드다. 돈모으기 탭 아이콘은 새싹(`Sprout`), 수입 탭 아이콘은 손모으기(`HandCoins`)다.
...
「상세 정보」 카드는 **메모·날짜**(자주 써서 항상 표시)와, 그 아래 **결제수단+공개범위를
한 묶음으로 접는 「결제·공개」 행**으로 구성한다. 이 묶음은 **기본 닫힘**이고 요약행에
현재값을 `결제(진한 색) │ 공개(연한 색)`으로 구분해 노출한다(예: 「신용카드 · 9개월 │
전체 공개」 — 결제 데이터와 공개범위는 다른 축이라 세로 구분선으로 나눈다). 펼치지 않아도
저장값을 알 수 있고, 자주 안 바꾸는 설정을 접어 폼 세로를 크게 줄인다. `›`를 탭하면
펼쳐지고(아래로 회전), 결제수단 2-세그먼트 토글과 공개범위 세그먼트가 나온다. 새 소비는
직전 사용 결제수단을 기본값으로 깐다(localStorage, 고정 기본값은 신용/체크 비율을
오염시키므로). 수정 시엔 해당 거래의 값을 채운다. 결제수단 컬럼 도입 전 legacy 거래는
값이 없어 §12.9에서 "미지정"으로 집계된다.

**할부** — 별도 할부 행을 두지 않는다. **신용카드 chip 자체가 할부 진입점**이다(생성
모드·비분할). 신용카드를 탭하면(체크→신용 전환이든 재탭이든) **nested drawer**가 바로
열리고, 그 안 **native select**(일시불 / 2~36개월, 옵션마다 「· 월 X원」 월 부담 표기)에서
개월을 고르면 즉시 적용 + 닫힘. 할부가 걸리면 신용카드 chip이 「신용카드 · N개월」로 바뀌어
현재 개월을 보여주고(「결제·공개」 요약행·금액 히어로에도 반영), 재탭으로 다시 고른다. 금액
히어로 큰 숫자(= 원금 총액) 아래에 「월 약 X원 × N개월」 힌트를 작게 표기한다. 체크로
바꾸면 일시불로 리셋. (마지막 결제수단이 localStorage에 기억되므로 신용 상시 유저는 재탭이
없어 자동 오픈이 성가시지 않다.) 할부는 회계 모델
...
**나눠내기(정산)** — 친구와 나눠 낸 소비의 **내 몫만** 기록하기 위한 기능. 금액 히어로
**좌상단 아이콘 버튼**(되돌리기/삭제 유틸과 대칭, 아이콘만)으로 진입한다 — 별도 행을 두지
않아 §3 핵심 숫자 우선을 지킨다. 분할하면 아이콘이 primary 색 + 우상단 인원수 배지로
바뀌고, 히어로 숫자가 곧 내 몫이라 "얼마"는 아이콘에서 따로 안 보여준다. 탭하면 **nested
drawer**가 열리고(공개범위 「부분」의 `GroupPickerDrawer`와 동일한 `DrawerNestedRoot`
패턴), 그 안 **native select**(안 나눔 / 2~10명, 옵션마다 「· 내 몫 X원」 표기, 단일 소스
`SPLIT_MAX_PEOPLE`)에서 인원을 고르면 즉시 적용 + 닫힘. 기준 총액 0원이면 아이콘은
비활성(dim)이다.

- **총액 입력 → 인원 선택 → 내 몫**: 금액칸에 결제한 총액을 넣고 select에서 인원(2~10명)을
  고르면 금액칸이 `round(총액/N)`(내 몫)으로 바뀐다. 「안 나눔」은 분할 해제(총액 복원).
- **내 몫만 저장**: `amount`엔 내 몫만 담기고, 총액(`split_total`)·인원(`split_count`)은
  ...
  **기존 쿼리는 이 행을 일반 거래로 그대로 취급**한다(할부 B1과 같은 철학 — 쿼리 무변경).
  ...
  `lib/utils/split.ts::computeShare`, DB CHECK: 둘 다 null 또는 둘 다 set·2~10·총액>0 — 0061에서 확장).
- **표시**: 내역 아이템은 카테고리/메모 아래 「총 X원 · N명 나눔」 한 줄(own 전용). **친구
  모드에서는 숨긴다** — 친구는 이미 내 몫(amount)을 보므로 총액·인원까지 노출하지 않는다.
- **할부와 상호배타(v1)**: 할부와 나눠내기는 함께 못 쓴다. 나눠내기가 켜지면(splitCount≥2)
  신용 chip의 할부 진입이, 할부가 켜지면(개월≥2) 나눠내기 아이콘이 숨는다(서버도 이중 방어).
  결제수단은 신용/체크 무관.
...
- 그 아래 **급여 규정은 기본 접힘**(점진적 노출 — 주말·공휴일 보정이 필요한 사람만 펼친다): 항상 보이는 행 "주말·공휴일 겹칠 때" 우측에 **현재값 라벨 + chevron**을 노출한다(안 펼쳐도 상태 확인 가능). 탭하면 RadioGroup 3옵션이 펼쳐진다 — `앞당겨 들어와요`(기본, prev, 헬퍼 "예: 토요일이면 금요일") / `날짜 그대로`(same) / `미뤄서 들어와요`(next, 헬퍼 "예: 토요일이면 월요일"). 추상 워딩("이전/다음 영업일")은 **펼친 picker의 옵션 라벨**에서 페르소나-중립 구체 표현으로 교체했다 — 단, **접힌 값 요약 row**(트리거 자체, §12.11 참조)는 공간이 좁아 짧은 원형 표기("N일 · 이전 영업일" 등, `PAYROLL_RULE_SHORT`)를 그대로 쓴다. 두 표기는 각자의 자리(펼친 옵션 vs 접힌 요약)에서 공존하는 의도된 차이다. **모드(달력형/세부형) 분리는 하지 않는다** — 모드 포크는 분류 질문을 강제해 단순 유저에게 메타 결정 세금을 물리므로, 점진적 노출이 우선이다.
...
- 진입점은 대시보드 합계 카드 탭 하나다 (§12.2, §11.2). 카드 하단의 「📊 소비 구성 보기 ›」 subtle 행이 진입 단서이며, 탭은 카드 전체가 받는다. 하단 탭에 추가하지 않는다 (4탭 유지, §19).
...
- **변동 카테고리 drill-down**: 변동 행을 탭하면 그 카테고리의 개별 거래를 아래로 펼친다(날짜·메모(있으면)·금액). 정렬은 섹션 헤딩과 리스트 카드 사이의 **섹션 전역 「금액순/날짜순」 텍스트 토글**로 전환한다 — 기본 **금액순**(금액 desc — 부모 카테고리 정렬과 같은 "큰 것부터", 동일금액은 최신순 tie-break), 날짜순 = 최신순(동시각은 금액 desc tie-break). 토글은 shadcn Tabs가 아닌 평문 버튼 2개(`aria-pressed`, 선택 쪽만 색/굵기 강조)이고, 상태는 세션 내 메모리만(localStorage 저장 없음 — 정렬은 탐색용 일시 상태). **exclusive accordion** — 한 번에 한 카테고리만 열리고, 새 행을 열면 이전 행이 닫힌다. 집계(카테고리로 묶기)는 그대로 두되 "그 안에 뭐가 있나"를 한 단계 확인하는 surface로, own 전용 화면이므로 거래 메모를 그대로 노출한다(친구 모드 진입점 없음). 고정 항목은 펼치지 않는다(항목 자체가 이미 최소 단위 — §12.9 비대칭). 거래 **수정/삭제는 여기서 하지 않는다** — drill-down은 읽기 전용, 편집은 대시보드의 거래 폼이 담당(surface 한 단계 유지 §19).
...
`/savings`는 매달 모으는 돈(적금·투자)을 관리한다. 고정지출이 "쓰고 사라지는 돈"이라면 돈모으기는 **"다시 내 자산이 되는 돈"**이다 — 의미가 달라 별도 탭으로 분리한다(§11.2). 데이터는 `fixed_expenses`와 별개의 `savings_plans` 테이블이며, 친구에게는 공개하지 않는다(수입·예산만큼 사적이다).
...
- **「모으는 중」 리스트**: active 항목을 **적립일 임박순**(`comparePaymentDayUpcoming`, payment_day 없으면 월 적립액 내림차순 폴백)으로 한 리스트에 보여준다. 각 행 = 이름 + 「매월 N일」(적립일 없으면 「적립일 미정」) + 월 적립액(없으면 「금액 미입력」). 만기가 있으면 보조 라인에 「만기까지 N개월」 + 그 아래 **시간 기반 진행바**(시작일→만기일 경과 비율, `bg-muted` 트랙 / `#1c8c4d`(모으기 초록) 채움, % 숫자는 없음) + 진행바 아래 **시작일·만기일을 양끝 정렬**(시작일 왼쪽 / 만기일 오른쪽, `justify-between`, muted) — 바의 채움이 시작→만기로 읽히게 앵커. 목표 금액 기반 진행률·누적(모은 돈) 표시는 없다(만기 진행바는 기간 경과일 뿐 목표 달성률이 아니다). 만기 없는 자유 적립/투자는 진행바가 없다. 고정지출 「사용 중」 리스트 미러(만기 진행바만 추가).
- **FAB**: 오른쪽 아래 추가 버튼(대시보드 FAB와 동일 위치·primary). 적금·투자 추가 BottomSheet 진입.
...
- 만기 있으면 「만기까지 N개월」 + 시간 기반 진행바(시작일→만기일 경과, `maturityProgressPct`). 목표금액형 진행률·「목표까지 약 N개월」은 제거됨 — 만기 진행바는 **기간 경과**이지 금액 달성률이 아니다.
- **색: 모으기 시맨틱 accent는 초록 `#1c8c4d`로 통일**(대시보드 「나간 돈」 모으기 세그먼트·달력 마커와 동일 색). /savings에선 히어로 「자산 되는 돈」 강조·「총 N개 항목」 배지 점·만기 진행바·빈 상태 Sprout가 초록. 단 **액션 컨트롤(FAB·추가/수정 버튼·빈 상태 CTA)은 `--primary` 파랑 유지** — 4탭 공통 "액션=파랑" 컨벤션을 지킨다(초록은 데이터/정체성, 파랑은 조작). `#1c8c4d`는 아직 토큰이 아니라 인라인(대시보드와 동일)이며, 추후 `--savings` 토큰화는 별도 정리.
...
`/income`는 이번 주기에 들어온 돈 전체 — 월 수입 + 추가 수입 — 를 한 화면에서 보여준다. 이전에는 온보딩·설정·대시보드 FAB 꾹 누르기·소비 요약 수입 라인 네 곳에 흩어져 있던 수입 표면을 이 탭 하나로 모은 것이다. 온보딩 2단계(최초 월 수입 입력)는 그대로 유지된다.

구성(위→아래, single column, `max-w-md`):

```txt
PageHeader  eyebrow="이번 주기 들어온 돈"  title="수입"
◀  주기 라벨 ("1월" / "M/D–M/D")  ▶   ← MonthSwitcher, 대시보드와 같은 엔진
┌ HERO 카드 ──────────────────┐
│ 이번 주기 총 수입              │
│ ₩3,250,000                  │  ← monthly_income + Σ추가수입 (§3 핵심 숫자)
│ 월 수입 300만원 · 추가 25만원  │  ← 보조 breakdown 한 줄
└──────────────────────────────┘
[월 수입]  "매달 들어오는 실수령 금액"  ₩3,000,000  ›   ← 현재 주기에서만 노출
[돈 들어오는 날]  1일 · 이전 영업일  ›               ← 예산 주기, 모든 주기에서 노출 (§12.5a, 접힌 요약 짧은 표기)
[추가 수입 N · 최신순]
...
- 소비·고정지출·돈모으기·수입 사이는 하단 4탭으로 이동한다 (§11.2).
...
- 하단 탭에 5개 이상의 항목을 넣지 않는다 (소비 / 고정지출 / 돈모으기 / 수입 네 개만). 설정·통계·친구는 탭에 넣지 않는다.

...
바로 아래 **접힌 토글**(「여러 명이 나눠 냈어요」)로 두고, 소비 대부분은 혼자이므로 기본은
접혀 있어 빠른입력 경로를 방해하지 않는다(§3 핵심 숫자 우선). 펼치면 `SplitChips`(고정지출
셰어 칩 재사용) 인원 칩 **혼자 다 / 2명 / 3명 / 4명**이 뜬다.
...
- **결제수단 섹션**: 변동 `transactions`를 `payment_method`(신용/체크/미지정)로 집계한 비율. 행 = 라벨·%·금액(막대 없음, 변동 섹션과 같은 미니멀). **분모는 변동지출 총액**(고정엔 결제수단이 없음)이라 SectionHeading 합계 = 변동 총액이고, 「변동지출 기준」 캡션으로 분모를 명시한다 — 상단 「총 소비」(고정+변동)와 다른 분모임을 분명히 한다. 신용/체크가 둘 다 0이고 미지정만 있으면(전부 legacy) 섹션을 숨긴다 — 쪼갤 게 없으면 노이즈다. 결제수단 컬럼 도입 전 거래는 `payment_method = null`이라 "미지정" 버킷에 모이며, 신규 거래가 쌓이면서 채워진다. 막대/도넛 차트는 두지 않는다(§19 Recharts 금지).

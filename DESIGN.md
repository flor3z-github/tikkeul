...
**할부** — 결제수단이 **신용카드**일 때만(생성 모드·비분할) 결제수단 아래에 「할부」
**값 요약 row**(「일시불」 / 「N개월 · 월 약 X원」)가 노출되고, 탭하면 **nested
drawer**의 개월 칩(일시불 / 2~36개월, 2개월 이상 칩엔 월 부담 서브라벨)이 열린다.
칩 탭 = 즉시 적용 + 닫기. 원금 0원 + 일시불이면 row 비활성(이미 할부가 걸려 있으면
되돌릴 수 있게 활성 유지). 체크로 바꾸면 일시불로 리셋. 할부는 회계 모델
...
바로 아래 **한 줄 트리거 행**(「여러 명이 나눠 냈어요」, 나눈 뒤엔 「총 X원 ÷ N명」 —
히어로 숫자가 이미 내 몫이라 트리거엔 총액을 보여준다)만 두고, 인원 칩은 **nested
drawer**로 뺀다(공개범위 「부분」의 `GroupPickerDrawer`와 동일한 `DrawerNestedRoot`
패턴) — 인라인 확장이 폼 세로를 크게 늘려 §3 핵심 숫자 우선을 해쳤기 때문. drawer 안에
`SplitChips`(고정지출 셰어 칩, `shares` prop으로 인원 주입) 인원 칩 **혼자 다 /
2~10명**(단일 소스 `SPLIT_MAX_PEOPLE`)을 띄우고, 칩을 누르면 즉시 적용하고 닫는다(단일
선택이라 버퍼/커밋 불필요). 기준 총액 0원이면 트리거 행은 비활성이다.

- **총액 입력 → 인원 선택 → 내 몫**: 금액칸에 결제한 총액을 넣고 인원 칩(2~10명)을 누르면

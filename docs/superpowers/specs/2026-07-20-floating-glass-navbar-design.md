...
DOM은 항상 **확장 상태 레이아웃**으로 렌더하고, 축소는 시각 효과로만 처리한다. 애니메이션 속성은 `clip-path` / `transform` / `opacity` 3종뿐이다.
...
1. **컨테이너 클리핑**: `clip-path: inset(var(--clip-y) var(--clip-x) round 999px)` — 시각적 폭·높이 축소. 확장 상태는 `inset(0 0 round 32px)`.
2. **탭 아이템 이동**: 4개 아이템 각각 `transform: translate(var(--tx-i), var(--ty))` — 바깥 2개는 크게, 안쪽 2개는 작게 중앙으로 모이고, 라벨이 사라진 만큼 아이콘이 수직 중앙으로 내려온다.
3. **라벨 페이드**: `opacity: 0`. 축소 시 120ms 선행 페이드아웃, 확장 시 ~100ms 지연 후 페이드인 (클리핑과 겹치는 순간 텍스트가 잘려 보이는 것 방지).
...
- `transition: clip-path 260ms cubic-bezier(0.32, 0.72, 0, 1), transform 260ms 동일 이징` (vaul과 동일 계열, 비-bouncy — DESIGN.md §8 "빠르고 조용해야 한다" 준수).
...
- pill이 클리핑돼도 실제 hit-area는 확장 레이아웃 그대로라는 점 주의: 축소 상태에서 클리핑된 바깥 영역이 투명하게 탭을 받으면 안 되므로, 축소 시 각 `Link`의 hit-area가 이동된 시각 위치와 일치하는지 POC에서 확인한다 (transform은 hit-area도 함께 이동시키므로 원칙적으로 일치).
